...
\begin{document}
    ...
    \begin{figure}[h]
        ...
        \small Hình 4: Các chỉ số hàm mất mát trên tập huấn luyện
    ...
    \begin{figure}[h]
        ...
        \small Hình 5: Các chỉ số hàm mất mát trên tập kiểm định
        \label{fig:val_loss}
    \end{figure}
	...
	\begin{figure}[h]
    	...
    	\small Hình 6: Biểu đồ mAP@50 của mô hình $M_{assist}$ qua các epoch
    ...
    Sau khi hoàn tất quy trình xây dựng dữ liệu thông qua cơ chế Human-in-the-loop, mô hình chính \textbf{YOLO26n} được huấn luyện để đánh giá khả năng nhận thức hành vi thực tế. Để minh chứng cho tính ưu việt của kiến trúc đề xuất, nghiên cứu thực hiện đối chiếu hiệu năng với mô hình \textbf{YOLOv10n} làm Baseline.

    \subsubsection{So sánh hiệu năng nhận diện và phân loại}

    Kết quả thực nghiệm trên tập kiểm chứng gồm 3.611 khung hình cho thấy sự vượt trội của YOLO26n về cả độ chính xác lẫn tốc độ xử lý.

    \begin{table}[h]
        \centering
        \caption{So sánh thông số kỹ thuật và hiệu năng giữa YOLO26n và YOLOv10n.}
        \label{tab:final_results}
        \resizebox{\linewidth}{!}{%
            \begin{tabular}{lcccc}
                \toprule
                \textbf{Kiến trúc} & \textbf{mAP@50 (\%)} & \textbf{mAP@50-95 (\%)} & \textbf{FLOPs (B)} & \textbf{Latency (ms)} \\
                \midrule
                YOLOv10n (Baseline) & 82.4 & 39.5 & 6.7 & 1.2 \\
                \textbf{YOLO26n (Ours)} & \textbf{89.7} & \textbf{40.9} & \textbf{5.4} & \textbf{0.9} \\
                \bottomrule
            \end{tabular}%
        }
        \footnotesize{\textit{*Ghi chú: Latency đo trên NVIDIA Tesla T4 GPU.}}
    \end{table}

    \textbf{Phân tích kết quả thực nghiệm:}
    \begin{itemize}
        \item \textbf{Độ chính xác:} YOLO26n đạt hiệu suất \textbf{89.7\% mAP@50}, vượt trội hơn đáng kể so với kiến trúc tiền nhiệm. Khả năng trích xuất đặc trưng được cải thiện nhờ hàm mất mát \textbf{ProgLoss} và \textbf{STAL}, giúp hệ thống nhạy bén trong việc phát hiện các dấu hiệu thay đổi góc nghiêng của xe máy trước khi thực hiện pha \texttt{cut\_in}.
        \item \textbf{Tối ưu hóa tài nguyên:} Mặc dù có số lượng tham số tương đương (~2.4M), YOLO26n giảm được lượng tính toán FLOPs xuống còn \textbf{5.4B}. Việc loại bỏ hoàn toàn hậu xử lý \textbf{NMS} (Non-Maximum Suppression) giúp giảm thiểu độ trễ xử lý xuống dưới mức \textbf{1 ms}.
    \end{itemize}

    \subsubsection{Đánh giá khả năng dự đoán ý định sớm}

    Mục tiêu quan trọng nhất của hệ thống là khả năng đưa ra cảnh báo trước khi hành vi nguy hiểm xảy ra hoàn toàn. Qua thực nghiệm trên các kịch bản giao thông hỗn hợp tại Việt Nam, hệ thống đã chứng minh được năng lực phản hồi sớm:

    \begin{itemize}
        \item \textbf{Thời gian phản ứng:} Nhờ sự kết hợp giữa kiến trúc End-to-End và bộ tối ưu hóa \textbf{MuSGD}, hệ thống có khả năng đưa ra cảnh báo sớm từ \textbf{1.5 đến 2 giây} trước khi xe máy cắt mặt ô tô.
        \item \textbf{Độ ổn định cảnh báo:} Thuật toán hậu xử lý giúp ổn định tín hiệu, giảm thiểu tình trạng cảnh báo giả do nhiễu môi trường hoặc các xe di chuyển phi làn đường nhưng không có ý định tạt đầu.
    \end{itemize}

    \textbf{Kết luận:} Kết quả thực nghiệm khẳng định YOLO26n là lựa chọn tối ưu cho hệ thống ADAS triển khai trên thiết bị biên. Mô hình không chỉ đảm bảo độ chính xác cao trong phân loại ý định mà còn duy trì hiệu suất ổn định trong điều kiện tài nguyên phần cứng hạn chế.
